...
{
    public partial class Form_BinSorter : Form, ITesterAppUI, IAccessPermissionLevel
    {

        public Form_BinSorter()
        {
            InitializeComponent();
        }
        ...
        BinSortManager _myBinSortManager;
        ...
        {
            _appInteration = app;
            _myBinSortManager = (BinSortManager)app;
        }
        ...
        {
            cb_BinToEdit.Items.Clear();
            var specNames = _myBinSortManager.GetBinSettingTags();
            foreach (var sName in specNames)
            {
                cb_BinToEdit.Items.Add(sName);
            }
            cb_BinToEdit.SelectedIndex = -1;
        ...
        private void cb_SpecToEdit_SelectionChangeCommitted(object sender, EventArgs e)
        {
            try
            {
                ...
                binSetName = cb_BinToEdit.SelectedItem.ToString();

                var binSetting = _myBinSortManager.GetBinByTag(binSetName);
                if (binSetting == null)
                {
                    MessageBox.Show($"未找到对应的规格 binSetting = {binSetName}!");
                    UpdateSpecsSelector();
                    return;
                }
                this.pdgv_specEditor.ImportSourceData<BinJudgeItem>(binSetting, true);
            }
            catch (Exception ex)
            {
        ...
        private void btn_addSpec_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string binSetName = string.Empty;
                if (cb_BinToEdit.SelectedItem == null ||
                string.IsNullOrEmpty(cb_BinToEdit.SelectedItem.ToString()))
                {
                ...
                else
                {
                    ...
                    if (binsetting != null)
                    {
                        ...
                        {//保存
                            ...
                            //替换保存
                            var result = _myBinSortManager.UpdateTestSpecInCollection(binSetName, newBinsetting, ref msg);
                            _myBinSortManager.Save_Collection();
                ...
                var orgBinNames = _myBinSortManager.GetBinSettingTags();
                ...
                _myBinSortManager.AddBinSettingToCollection(newBin);
                UpdateSpecsSelector();
                cb_BinToEdit.SelectedItem = newBin.Name;
        ...
        private void btn_deleteSpec_Click(object sender, EventArgs e)
        {
            ...
            var binName = cb_BinToEdit.SelectedItem.ToString();
            try
            {
        ...
        private void btn_copySpec_Click(object sender, EventArgs e)
        {
           ...
           string.IsNullOrEmpty(cb_BinToEdit.SelectedItem.ToString()))
            ...
            {
                //0418XINZENG
                sourceBin = _myBinSortManager.GetBinByTag(binName);
                newBin = new BinSetting(sourceBin);
                DateTime dateTime = DateTime.Now;
                newBin.Name = "SampleBin_" + dateTime.ToString("MMddHHmmssss");

                this.pdgv_specEditor.Clear();
                this.cb_BinToEdit.Text = string.Empty;

                this.cb_BinToEdit.SelectedIndex = -1;// = string.Empty;
            ...
            //0418
            this.cb_BinToEdit.Text = newBin.BinTag;
            this.pdgv_specEditor.ImportSourceData<BinJudgeItem>(newBin, true);
        ...
        private void btn_saveSpec_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string.IsNullOrEmpty(cb_BinToEdit.SelectedItem.ToString()))
                {
                    this._myBinSortManager.Save_Collection();
                }
                else
                {
                    binSetName = cb_BinToEdit.SelectedItem.ToString();
        ...
        private void SaveAndUpdate(string binName,    BinSetting spec)
        {
            ...
            try
            {
                ...
                var result = _myBinSortManager.UpdateTestSpecInCollection(binName,spec, ref msg);
        ...
        private void DelAndUpdate(BinSetting spec, bool isExist)
        {
            try
            {
                string msg = string.Empty;
                //文件中无
                var result = _myBinSortManager.DeleteTestSpecFromCollection(spec, ref msg);
                if (!result)
                {
        ...
        private void DelAndUpdate(string binName/*, bool isExist*/)
        {
            try
            {
                string msg = string.Empty;
                //文件中无
                var result = _myBinSortManager.DeleteTestSpecFromCollection(binName, ref msg);
                if (!result)
                {
                    throw new Exception(msg);
                }
