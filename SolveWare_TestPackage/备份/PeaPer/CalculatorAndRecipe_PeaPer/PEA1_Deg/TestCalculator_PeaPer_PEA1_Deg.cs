...
namespace SolveWare_TestPackage
{
    ...
    /// 
    /// </summary>
    public class TestCalculator_PeaPer_PEA1_Deg : TestCalculatorBase
    {
        public TestCalculator_PeaPer_PEA1_Deg() : base() { } 
        public CalcRecipe_PeaPer_PEA1_Deg CalcRecipe { get; private set; }
        public override Type GetCalcRecipeType() { return typeof(CalcRecipe_PeaPer_PEA1_Deg); }
        public override void Localization(ICalcRecipe testRecipe) { CalcRecipe = (CalcRecipe_PeaPer_PEA1_Deg)testRecipe; }
        public override void Run(IRawDataBaseLite rawData, ref List<SummaryDatumItemBase> summaryDataWithoutSpec, CancellationToken token)
        {
            try
            {
                ...
                {
                    dataSummary_1.Name = string.Format(rawData.RawDataFixFormat, $"{CalcRecipe.CalcData_PreFix}PEA1_Deg{CalcRecipe.CalcData_PostFix}");
                }
                ...
                double resultCurrent = PeaPerCalcuator.CalculatotAllData_ReturnSingle(Curve_Fit, UsePDCurrentCalculatePER, dict[CurrentTag], dict[PowerTag], dict[Angle]).PEA1_Deg;
                dataSummary_1.Value = resultCurrent; 
            }
            catch (Exception ex)
            {
                this._core.ReportException($"{this.Name} - {ex.Message}", ErrorCodes.Calc_PER_PEA1_Deg_ParamError, ex);
            }
        }      
    }
}