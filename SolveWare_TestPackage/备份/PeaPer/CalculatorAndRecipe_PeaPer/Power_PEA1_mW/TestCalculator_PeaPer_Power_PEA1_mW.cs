...
namespace SolveWare_TestPackage
{
    ...
    /// 
    /// </summary>
    public class TestCalculator_PeaPer_Power_PEA1_mW : TestCalculatorBase
    {
        public TestCalculator_PeaPer_Power_PEA1_mW() : base() { } 
        public CalcRecipe_PeaPer_Power_PEA1_mW CalcRecipe { get; private set; }
        public override Type GetCalcRecipeType() { return typeof(CalcRecipe_PeaPer_Power_PEA1_mW); }
        public override void Localization(ICalcRecipe testRecipe) { CalcRecipe = (CalcRecipe_PeaPer_Power_PEA1_mW)testRecipe; }
        public override void Run(IRawDataBaseLite rawData, ref List<SummaryDatumItemBase> summaryDataWithoutSpec, CancellationToken token)
        {
            try
            {
                ...
                {
                    dataSummary_1.Name = string.Format(rawData.RawDataFixFormat, $"{CalcRecipe.CalcData_PreFix}Power_PEA1_mW{CalcRecipe.CalcData_PostFix}");
                }
                ...
                double resultCurrent = PeaPerCalcuator.CalculatotAllData_ReturnSingle(Curve_Fit, UsePDCurrentCalculatePER, dict[CurrentTag], dict[PowerTag], dict[Angle]).Power_PEA1_mW;
                dataSummary_1.Value = resultCurrent; 
            }
            catch (Exception ex)
            {
                this._core.ReportException($"{this.Name} - {ex.Message}", ErrorCodes.Calc_PER_Power_PEA1_mW_ParamError, ex);
            }
        }        
    }
}