...
namespace SolveWare_TestPackage
{
    ...
    /// 
    /// </summary>
    public class TestCalculator_NF_NF : TestCalculatorBase
    {
        public TestCalculator_NF_NF() : base() { }   
        public CalcRecipe_NF_NF CalcRecipe { get; private set; } 
        public override Type GetCalcRecipeType() { return typeof(CalcRecipe_NF_NF); }
        public override void Localization(ICalcRecipe testRecipe) { CalcRecipe = (CalcRecipe_NF_NF)testRecipe; }
        public override void Run(IRawDataBaseLite rawData, ref List<SummaryDatumItemBase> summaryDataWithoutSpec, CancellationToken token)
        {
            try
            {
                ...
                {
                    dataSummary_1.Name = string.Format(rawData.RawDataFixFormat, $"{CalcRecipe.CalcData_PreFix}NF_NF{CalcRecipe.CalcData_PostFix}");
                }
                dataSummary_1.Value = 0.0;
                summaryDataWithoutSpec.Add(dataSummary_1);

                // dataSummary_1.Value = (rawData as RawData_NF).NF;

                const string Wavelength_nmTag = "Wavelength_nm";
                const string NF_dBTag = "NF_dB";

                var localRawData = rawData as IRawDataCollectionBase;
                var dict = localRawData.GetDataDictByPropNames<double>(Wavelength_nmTag, NF_dBTag);

                if (dict[Wavelength_nmTag]?.Count <= 0 || dict[NF_dBTag]?.Count <= 0 | dict[Wavelength_nmTag]?.Count != dict[NF_dBTag]?.Count)
                {
                ...
                if (this.CalcRecipe.USE_Specified_Wavelength_nm)
                {
                    ...
                    double resultCurrent = GetCurrentForSpecifiedPower(dict[Wavelength_nmTag], dict[NF_dBTag], Wavelength_nm);
                   // double resultCurrent = Interpolator.DoPiecewiseLinearInterpolation(Wavelength_nm, dict[Wavelength_nmTag].ToArray(), dict[NF_dBTag].ToArray());
                    dataSummary_1.Value = resultCurrent;
                }
                else
                {
                    double Wavelength_nm = (rawData as RawData_NF).TraceA_CenterWavelength_nm;
                   // double resultCurrent = GetCurrentForSpecifiedPower(dict[NF_dBTag], dict[Wavelength_nmTag],  Wavelength_nm);
                    double resultCurrent = Interpolator.DoPiecewiseLinearInterpolation(Wavelength_nm, dict[Wavelength_nmTag].ToArray(), dict[NF_dBTag].ToArray());
            ...
            {
                this._core.ReportException($"{this.Name} - {ex.Message}", ErrorCodes.Calc_NF_NF_ParamError, ex);
            }
        }
        ...
        public double GetNFForSpecifiedWavelength(List<double> wavelengthlist, List<double> nflist, double wavelength_nm) 
        {
            double nf = 0;
            try
            {
                if (wavelengthlist.Count == nflist.Count)
                {
