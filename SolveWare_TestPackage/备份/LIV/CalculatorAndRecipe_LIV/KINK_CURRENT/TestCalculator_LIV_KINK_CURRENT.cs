...
namespace SolveWare_TestPackage
{
    [Serializable]
    /// <summary>
    /// Kink_Current算子 CT3102
    /// 找出目标电流点下对应的器件输出功率
    /// </summary>
    public class TestCalculator_LIV_Kink_Current : TestCalculatorBase
    {
        public TestCalculator_LIV_Kink_Current() : base() { }

        public CalcRecipe_LIV_Kink_Current CalcRecipe { get; private set; }

        public override Type GetCalcRecipeType()
        {
            return typeof(CalcRecipe_LIV_Kink_Current);
        }

        public override void Localization(ICalcRecipe testRecipe)
        {
            CalcRecipe = (CalcRecipe_LIV_Kink_Current)(testRecipe);
        }
        ...
        public override void Run(IRawDataBaseLite rawData, ref List<SummaryDatumItemBase> summaryDataWithoutSpec, CancellationToken token)
        {
            try
            {
                ...
                {
                    dataSummary_1.Name = string.Format(rawData.RawDataFixFormat, $"{CalcRecipe.CalcData_PreFix}Kink_Current{CalcRecipe.CalcData_PostFix}");
                }
                ...
                {
                    throw new Exception($"Kink_Current: xArray and yArray are of unequal size!");
                }
                ...
                //最后赋值
                dataSummary_1.Value = KinkCurrent;
             

            }
            catch (Exception ex)
            {
                this._core.ReportException($"{this.Name} - {ex.Message}", ErrorCodes.Calc_LIV_Kink_Current_ParamError, ex);
            }
        }
    }
}