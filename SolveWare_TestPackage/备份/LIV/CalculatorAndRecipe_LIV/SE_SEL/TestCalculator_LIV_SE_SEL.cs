...
namespace SolveWare_TestPackage
{
    ...
    /// 找出SE最大值
    /// </summary>
    public class TestCalculator_LIV_SE_SEL : TestCalculatorBase
    {
        public TestCalculator_LIV_SE_SEL() : base() { }

        public CalcRecipe_LIV_SE_SEL CalcRecipe { get; private set; }

        public override Type GetCalcRecipeType()
        {
            return typeof(CalcRecipe_LIV_SE_SEL);
        }

        public override void Localization(ICalcRecipe testRecipe)
        {
            CalcRecipe = (CalcRecipe_LIV_SE_SEL)(testRecipe);
        }
        ...
        public override void Run(IRawDataBaseLite rawData, ref List<SummaryDatumItemBase> summaryDataWithoutSpec, CancellationToken token)
        {
            try
            {
                ...
                {
                    dataSummary_1.Name = string.Format(rawData.RawDataFixFormat, $"{CalcRecipe.CalcData_PreFix}SE_SEL{CalcRecipe.CalcData_PostFix}");
                }
                ...
                {
                    throw new Exception($"SE_SEL: xArray and yArray are of unequal size!");
                }
                ...
                double current1 = ith2 + this.CalcRecipe.SE_Find_AboveIthCurrent;
                double current2 = this.CalcRecipe.SE_Find_MaxCurrent;
                double[] SElist_All = ArrayMath.CalculateFirstDerivate(dict[CurrentTag].ToArray(), dict[PowerTag].ToArray());
                double SEa = Interpolator.DoPiecewiseLinearInterpolation(current1, dict[CurrentTag].ToArray(), SElist_All);
                double SEb = Interpolator.DoPiecewiseLinearInterpolation(current2, dict[CurrentTag].ToArray(), SElist_All);
                dataSummary_1.Value = Math.Round((SEa - SEb) / SEa, 3);
                ...
                //double SEaCurrent = ith2 + this.CalcRecipe.SE_Find_AboveIthCurrent;
                //double SEbCurrent = this.CalcRecipe.SE_Find_MaxCurrent;
                //double SEa = GetSlopeEfficiencyForSpecifiedCurrent(sorted_SE, sorted_I, SEaCurrent);
                //double SEb = GetSlopeEfficiencyForSpecifiedCurrent(sorted_SE, sorted_I, SEbCurrent);
                //dataSummary_1.Value = Math.Round((SEa - SEb) / SEa, 3);
                #endregion
            ...
            {
                this._core.ReportException($"{this.Name} - {ex.Message}", ErrorCodes.Calc_LIV_SE_SEL_ParamError, ex);
            }
        }
