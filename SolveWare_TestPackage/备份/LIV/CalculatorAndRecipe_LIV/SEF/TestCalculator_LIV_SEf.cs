...
namespace SolveWare_TestPackage
{
    ...
    /// 找出目标功率点下对应的斜率效率
    /// </summary>
    public class TestCalculator_LIV_SEf : TestCalculatorBase
    {
        public TestCalculator_LIV_SEf() : base() { }

        public CalcRecipe_LIV_SEf CalcRecipe { get; private set; }

        public override Type GetCalcRecipeType()
        {
            return typeof(CalcRecipe_LIV_SEf);
        }

        public override void Localization(ICalcRecipe testRecipe)
        {
            CalcRecipe = (CalcRecipe_LIV_SEf)(testRecipe);
        }
        ...
        public override void Run(IRawDataBaseLite rawData, ref List<SummaryDatumItemBase> summaryDataWithoutSpec, CancellationToken token)
        {
            try
            {
                ...
                {
                    dataSummary_1.Name = string.Format(rawData.RawDataFixFormat, $"{CalcRecipe.CalcData_PreFix}SEf{CalcRecipe.CalcData_PostFix}");
                }
                ...
                {
                    throw new Exception($"SEf: xArray and yArray are of unequal size!");
                }
                ...
                dataSummary_1.Value = GetSlopeEfficiencyForSpecifiedCurrent(sorted_SE, sorted_I, CalcRecipe.Current_mA);
            }
            catch (Exception ex)
            {
                this._core.ReportException($"{this.Name} - {ex.Message}", ErrorCodes.Calc_LIV_SEf_ParamError, ex);
            }
        }
