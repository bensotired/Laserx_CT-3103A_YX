...
{
    [Serializable]
    public class TestCalculator_Spect_CenterWavelength : TestCalculatorBase
    {
        public TestCalculator_Spect_CenterWavelength() : base() { }

        public CalcRecipe_Spect_CenterWavelength CalcRecipe { get; private set; }

        public override Type GetCalcRecipeType()
        {
            return typeof(CalcRecipe_Spect_CenterWavelength);
        }

        public override void Localization(ICalcRecipe testRecipe)
        {
            CalcRecipe = (CalcRecipe_Spect_CenterWavelength)testRecipe;
        }
        public override void Run(IRawDataBaseLite rawData, ref List<SummaryDatumItemBase> summaryDataWithoutSpec, CancellationToken token)
        {
            try
            {
                ...
                {
                    dataSummary_1.Name = string.Format(rawData.RawDataFixFormat, $"{CalcRecipe.CalcData_PreFix}CenterWavelength{CalcRecipe.CalcData_PostFix}");
                }
                ...
                {
                    throw new Exception($"CenterWavelength:  xArray and yArray are of unequal size!");
                }
                double level = this.CalcRecipe.level;
                double halfMaxPowerWidth = CalculateHalfMaxPowerWidth(dict[WavelengthTag], dict[PowerTag], level);
                dataSummary_1.Value = halfMaxPowerWidth;
        ...
        private double CalculateHalfMaxPowerWidth(List<double> wavelengths_nm ,List<double> powers_mw,double level)
        {
            ...
            var ret = Math.Abs(right_wavelength + left_wavelength)/2;
            return ret;
        }
    }
}