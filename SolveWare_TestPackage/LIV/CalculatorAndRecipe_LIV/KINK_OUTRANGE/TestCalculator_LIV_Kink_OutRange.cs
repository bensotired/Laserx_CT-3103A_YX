...
namespace SolveWare_TestPackage
{
    ...
    /// 找出目标电流点下对应的器件输出功率
    /// </summary>
    public class TestCalculator_LIV_Kink_OutRange : TestCalculatorBase
    {
        public TestCalculator_LIV_Kink_OutRange() : base() { }

        public CalcRecipe_LIV_Kink_OutRange CalcRecipe { get; private set; }

        public override Type GetCalcRecipeType()
        {
            return typeof(CalcRecipe_LIV_Kink_OutRange);
        }

        public override void Localization(ICalcRecipe testRecipe)
        {
            CalcRecipe = (CalcRecipe_LIV_Kink_OutRange)(testRecipe);
        }
        ...
        public override void Run(IRawDataBaseLite rawData, ref List<SummaryDatumItemBase> summaryDataWithoutSpec, CancellationToken token)
        {
            try
            {
                ...
                {
                    dataSummary_1.Name = string.Format(rawData.RawDataFixFormat, $"{CalcRecipe.CalcData_PreFix}Kink_OutRange{CalcRecipe.CalcData_PostFix}");
                }
                ...
                {
                    throw new Exception($"Kink_OutRange: xArray and yArray are of unequal size!");
                }
                ...
                double kinkRange = (double)CalcRecipe.KinkOutRangeValue;   //KINK超限的值
                ...
                for (int i = 0; i < SE.Length - 1; i++)
                {
                    ...
                    {
                        //找到最大偏差的位置
                        SE_Diff_Max = Math.Abs(SE[i] - fittedSE);//只要超出范围值就直接跳出
                        MaxKink_Index = i;
                        KinkPercentage = (SE[i] - fittedSE) / fittedSE * 100; //百分比
                        KinkPower = powerArr[i]; //kink时候的功率
                        KinkCurrent = currentArr[i]; //kink时候的电流
                        if (Math.Abs(KinkPercentage) > kinkRange)
                        {
                            dataSummary_1.Value = KinkCurrent;//需要得到的是超过范围时候的第一个电流点
                            break;
                        }
                    }
                }
            ...
            {
                this._core.ReportException($"{this.Name} - {ex.Message}", ErrorCodes.Calc_LIV_Kink_OutRange_ParamError, ex);
            }
        }
