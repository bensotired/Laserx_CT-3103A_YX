...
namespace SolveWare_TestPackage
{
    ...
    /// 找出目标功率点下对应的驱动电流值
    /// </summary>
    public class TestCalculator_LIV_PowerMax : TestCalculatorBase
    {
        public TestCalculator_LIV_PowerMax() : base() { }

        public CalcRecipe_LIV_PowerMax CalcRecipe { get; private set; }

        public override Type GetCalcRecipeType()
        {
            return typeof(CalcRecipe_LIV_PowerMax);
        }

        public override void Localization(ICalcRecipe testRecipe)
        {
            CalcRecipe = (CalcRecipe_LIV_PowerMax)testRecipe;
        }
        public override void Run(IRawDataBaseLite rawData, ref List<SummaryDatumItemBase> summaryDataWithoutSpec, CancellationToken token)
        {
            try
            {
                ...
                //找出目标功率点下对应的驱动电流值
                dataSummary_1.Value = Pout_Max_mW;
     
             
            }
            catch (Exception ex)
            {
                this._core.ReportException($"{this.Name} - {ex.Message}", ErrorCodes.Calc_LIV_PowerMax_ParamError, ex);
            }
        }
    }
}