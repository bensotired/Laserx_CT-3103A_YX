...
namespace SolveWare_TestPackage
{
    ...
    /// 找出目标电流点下对应的器件输出功率
    /// </summary>
    public class TestCalculator_LIV_Kink_Power : TestCalculatorBase
    {
        public TestCalculator_LIV_Kink_Power() : base() { }

        public CalcRecipe_LIV_Kink_Power CalcRecipe { get; private set; }

        public override Type GetCalcRecipeType()
        {
            return typeof(CalcRecipe_LIV_Kink_Power);
        }

        public override void Localization(ICalcRecipe testRecipe)
        {
            CalcRecipe = (CalcRecipe_LIV_Kink_Power)(testRecipe);
        }
        ...
        public override void Run(IRawDataBaseLite rawData, ref List<SummaryDatumItemBase> summaryDataWithoutSpec, CancellationToken token)
        {
            try
            {
                ...
                {
                    dataSummary_1.Name = string.Format(rawData.RawDataFixFormat, $"{CalcRecipe.CalcData_PreFix}Kink_Power{CalcRecipe.CalcData_PostFix}");
                }
                ...
                {
                    throw new Exception($"Kink_Power: xArray and yArray are of unequal size!");
                }
                ...
                //最后赋值
                dataSummary_1.Value = KinkPower;
           

            }
            catch (Exception ex)
            {
                this._core.ReportException($"{this.Name} - {ex.Message}", ErrorCodes.Calc_LIV_Kink_Power_ParamError, ex);
            }
        }
    }
}