...
namespace SolveWare_TestPackage
{
    ...
    /// </summary>
    public class TestCalculator_LIV_Rs_V : TestCalculatorBase
    {
        public TestCalculator_LIV_Rs_V() : base() { }

        public CalcRecipe_LIV_Rs_V CalcRecipe { get; private set; }

        public override Type GetCalcRecipeType()
        {
            return typeof(CalcRecipe_LIV_Rs_V);
        }

        public override void Localization(ICalcRecipe testRecipe)
        {
            CalcRecipe = (CalcRecipe_LIV_Rs_V)(testRecipe);
        }
        ...
        public override void Run(IRawDataBaseLite rawData, ref List<SummaryDatumItemBase> summaryDataWithoutSpec, CancellationToken token)
        {
            try
            {
                ...
                {
                    dataSummary_1.Name = string.Format(rawData.RawDataFixFormat, $"{CalcRecipe.CalcData_PreFix}Rs_V{CalcRecipe.CalcData_PostFix}");
                }
                ...
                {
                    throw new Exception($"Rs_V: xArray and yArray are of unequal size!");
                }
                ...
                {
                    if (dict[VoltageTag][i] < this.CalcRecipe.StartVoltage_V || dict[VoltageTag][i] > this.CalcRecipe.StopVoltage_V)
                    {
                        continue;
                    }
            ...
            {
                this._core.ReportException($"{this.Name} - {ex.Message}", ErrorCodes.Calc_LIV_Rs_V_ParamError, ex);
            }
        }
    }
}