...
namespace SolveWare_TestPackage
{
    ...
    /// 找出目标功率点下对应的斜率效率
    /// </summary>
    public class TestCalculator_LIV_SEop : TestCalculatorBase
    {
        public TestCalculator_LIV_SEop() : base() { }

        public CalcRecipe_LIV_SEop CalcRecipe { get; private set; }

        public override Type GetCalcRecipeType()
        {
            return typeof(CalcRecipe_LIV_SEop);
        }

        public override void Localization(ICalcRecipe testRecipe)
        {
            CalcRecipe = (CalcRecipe_LIV_SEop)(testRecipe);
        }
        ...
        public override void Run(IRawDataBaseLite rawData, ref List<SummaryDatumItemBase> summaryDataWithoutSpec, CancellationToken token)
        {
            try
            {
                ...
                {
                    dataSummary_1.Name = string.Format(rawData.RawDataFixFormat, $"{CalcRecipe.CalcData_PreFix}SEop{CalcRecipe.CalcData_PostFix}");
                }
                ...
                {
                    throw new Exception($"SEop: xArray and yArray are of unequal size!");
                }

                Dictionary<double, double> dldi = new Dictionary<double, double>();
                List<double> sorted_Pow = new List<double>();
                ...
                dataSummary_1.Value = GetSlopeEfficiencyForSpecifiedPower(sorted_SE, sorted_Pow, CalcRecipe.Power_mW);
            }
            catch (Exception ex)
            {
                this._core.ReportException($"{this.Name} - {ex.Message}", ErrorCodes.Calc_LIV_SEop_ParamError, ex);
            }
        }
