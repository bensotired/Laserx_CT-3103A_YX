...
namespace SolveWare_TesterCore
{
   // IRawDataCollectionBase where TRawDatumItem : class, IRawDatumItemBase
    public partial class Form_DynamicTestExecutorDebugger_SimpleMode  : Form    , ITesterCoreLink 
    {
        ITesterCoreInteration _core;
        public Form_DynamicTestExecutorDebugger_SimpleMode( )
        {
            InitializeComponent();
           
        }

         IForm_RawDataViewer _dataUI;
        ...
        {
            if(this._dataUI == null)
            {
                throw new NotImplementedException("未注入Raw data UI!");
            }
        ...
        public bool Setup(object eciObj , object tstParamObj)
        {
            ...
            var  moduleType = this._core.GetTypeFromClassInPreLoadDlls(eci.TestModuleClassName);
            Form formObj = null;
            if (PropHelper.IsClassTypeBelongs<CustomizeRawDataUIAttribute>(moduleType) == true)
            {
                var uiName = PropHelper.GetAttributeValue<CustomizeRawDataUIAttribute>(moduleType).RawDataUI_TypeName;
                formObj = this._core.CreateInstanceClassInAppPluginDlls<Form>(uiName, null);
            }
            else
            {
                formObj = new Form_RawDataViewer();
            }
            this._core.ModifyDockableUI(formObj, true);
            pnl_data.Controls.Add(formObj);
            formObj.Show();


            this._dataUI = formObj as IForm_RawDataViewer;
            return true;
        }
        ...
        private void Form_TestExecutorDebugger_FormClosing(object sender, FormClosingEventArgs e)
        {
            ...
            if (_dataUI is Form_RawDataViewer_FF)
            {
                (_dataUI as Form_RawDataViewer_FF).Dispose();
            }
