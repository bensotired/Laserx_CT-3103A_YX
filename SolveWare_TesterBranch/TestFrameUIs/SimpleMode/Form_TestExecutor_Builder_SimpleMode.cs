...
{
    public partial class Form_TestExecutor_Builder_SimpleMode : Form, ITesterAppUI
    {
        ...
        public Form_TestExecutor_Builder_SimpleMode()
        {
            InitializeComponent();
        }
        ...
        private void btn_SaveTestExecutorConfigTree_Click(object sender, EventArgs e)
        {
            ...
            var filePath = _core.StationConfig.GetSystemParamsValue(TesterKeyPathsAndFiles.TEST_EXECUTOR_FILES_PATH);
        ...
        private void btn_UpdateCalculatorName_Click(object sender, EventArgs e)
        {
            try
            {
                if (this.treeView_TestExecutorConfigItem.SelectedNode == null)
                ...
                if( string.IsNullOrEmpty( this.tb_CalculatorParamNewName.Text) == true)
                {
                    MessageBox.Show($"算子参数名为空!"); return;
                }
                ...
                if (this.treeView_TestExecutorConfigItem.SelectedNode?.Level == lvl_calcNode)
                {
                    ...
                    if (this._editExecutorConfigItem.CalculatorCollection.IsIndexInRange(nodeIndex))
                    {
                        this._editExecutorConfigItem.CalculatorCollection.Book[nodeIndex].Value = tb_CalculatorParamNewName.Text;//.RemoveAtIndex(nodeIndex);
                        this.UpdateTreeView_ExecutorConfigItem(this.treeView_TestExecutorConfigItem, this._editExecutorConfigItem);
                        this.ExpandAllTree(treeView_TestExecutorConfigItem);
                    }
                    else
            ...
            {
                MessageBox.Show($"更新算子参数名错误:[{ex.Message}-{ex.StackTrace}]!");
            }
        }

        private void cms_listView_ItemClicked(object sender, ToolStripItemClickedEventArgs e)
        {
        ...
        private void toolStripMenuItem1_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    var dr = MessageBox.Show(
                                         $"确定删除{ listView_TestExecutorConfig.SelectedItems[0].Text}?",
                                         "删除操作",
                                         MessageBoxButtons.YesNoCancel,
                                         MessageBoxIcon.Question
                                     );
