...
namespace SolveWare_TestPackage
{
   ...
   // [StaticResource(ResourceItemType.POS, "FF_V_Zero_Pos", "FF垂直测试产品零点")]
    public partial class TestModule_FarField_V : TestModuleBase
    {
        public TestModule_FarField_V() : base()
        {
        }
        ...
        public override void Run(CancellationToken token)
        {
            try
            {
                ...
                int[] relayvalue = new int[] { 0, 0, 0, 1, 0, 0, 0, 1 };
                for (int i = 0; i < 8; i++)
                {
                    if (relayvalue[i] == 1)
                    {
                ...
                axisDict.Add(SX, Action_FF_Axis_V);

                Action_FF_Axis_H.SingleAxisHome(this.SX);
                Action_FF_Axis_V.SingleAxisHome(this.SZ);
                ...
                Action_FF_Axis_V.SingleAxisMotion(this.SX, actAngle, SpeedLevel.Normal);

                double logAngle = TestRecipe.Start_Angle;
                RawDatumItem_FarField rawDatumItem = new RawDatumItem_FarField();

                //单轴插补
                var actStartAngle = Zero_Pos.ItemCollection.Find(item => item.Name == "SX").Position;
                AxesPosition P1 = CloneHelper.Clone<AxesPosition>(Zero_Pos);
                AxesPosition P2 = CloneHelper.Clone<AxesPosition>(Zero_Pos);
                P1.ItemCollection.Find(item => item.Name == "SX").Position += TestRecipe.Start_Angle;
                P2.ItemCollection.Find(item => item.Name == "SX").Position += TestRecipe.Stop_Angle;
                //正反扫曲线
                ...
                trajDict.Add(PmTrajAxisType.B_Dir, this.SX);
                ...
                //回零
                Action_FF_Axis_V.SingleAxisMotion(this.SX, 0, SpeedLevel.Normal);
