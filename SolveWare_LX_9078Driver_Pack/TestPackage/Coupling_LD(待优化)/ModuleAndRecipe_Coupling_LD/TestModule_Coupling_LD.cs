...
namespace SolveWare_TestPackage
{
    ...
    public partial class TestModule_Coupling_LD : TestModuleBase
    {
        ...
        //private AxesPosition Start_Pos
        //{ get { return (AxesPosition)this.ModuleResource[ResourceItemType.POS][TestRecipe.Position.ToString()]; } }
        //{
        //    get
        //    {
        //        if (TestRecipe.ProductModel == TestRecipe_Coupling_LD.Product.DFB)
        //        {
        //            return (AxesPosition)this.ModuleResource[ResourceItemType.POS]["DFB光纤耦合点"];
        //        }
        //        else
        //        {
        //            return (AxesPosition)this.ModuleResource[ResourceItemType.POS]["VCSEL光纤耦合点"];
        //        }
        //    }
        //}
        ...
        private TestRecipe_Coupling_LD TestRecipe { get; set; }

        public override Type GetTestRecipeType()
        {
            return typeof(TestRecipe_Coupling_LD);
        }

        public override void Localization(ITestRecipe testRecipe)
        {
            TestRecipe = ConvertObjectTo<TestRecipe_Coupling_LD>(testRecipe);
        }

        private RawData_Coupling_LD RawData { get; set; }

        public override IRawDataBaseLite CreateRawData()
        {
            RawData = new RawData_Coupling_LD(); return RawData;
        }
        ...
        public override void Run(CancellationToken token)
        {
            ...
            //切换继电器
            int[] relayvalue = new int[] { 0, 1, 0, 0, 0, 1, 0, 0 };
            for (int i = 0; i < 8; i++)
            {
                if (relayvalue[i] == 0)
                {
            ...
            Keithley_2401.SourceMode = SourceModeTypes.Current;
            Keithley_2401.IsCurrentSourceAutoRangeOn = true;
            Keithley_2401.SenseMode = SenseModeTypes.Voltage;
            Keithley_2401.IsVoltageSenseAutoRangeOn = false;

            Keithley_2401.VoltageCompliance_V = TestRecipe.Compliance_V;
            Keithley_2401.CurrentSetpoint_A = TestRecipe.Current_mA / 1000;
            ...
            try
            {
                ...
                if (maxStep >= 1)
                {
                    ...
                    try
                    {
                        if (firstFineScan)
                        {
                            ...
                            var id = -1;
                            P1.ItemCollection.Find(item => item.Name == "TPSY").Position -= (id - 0) * TestRecipe.Layer_Step;
                            Log_Global($"开始精耦合[{id}]");
                            result = Run_Involute(eRunSize_Table.Fine, P1, UsedPlane, token);
                    ...
                    try
                    {
                        // if (needReverse)
                        {
                            ...
                            for (; -maxStep <= i && i <= maxStep;)
                            {
                                ...
                                var id = i;
                                P1.ItemCollection.Find(item => item.Name == "TPSY").Position = maxList[0].Position.GetSingleItem("TPSY").Position - id * TestRecipe.Layer_Step;
                ...
                if (this.TestRecipe.CrossPoint_Times > 0)
                {
                    ...
                    {
                        RawData.Position_X_mm =Math.Round(PointSearchResult.First().Key.ItemCollection.Find(item => item.Name == "TPSX").Position,6);
                        RawData.Position_Y_mm =Math.Round(PointSearchResult.First().Key.ItemCollection.Find(item => item.Name == "TPSY").Position,6);
                        RawData.Position_Z_mm =Math.Round(PointSearchResult.First().Key.ItemCollection.Find(item => item.Name == "TPSZ").Position,6);
        ...
        public bool DataAnalyze(TrajResultItem result, bool SaveRawData, out Dictionary<AxesPosition, double> AnalyzeResult)
        {
            try
            {
                ...
                if (SaveRawData)
                {
                    for (int i = 0; i < pList.Count; i++)
                    {
                        RawDatumItem_Coupling_LD rawdatumitem = new RawDatumItem_Coupling_LD();
                ...
                //20230224 林斌用面积中心做中心
                //var maxIndex = GetMax(pList);
                //Dictionary<AxesPosition, double> maxPoint = new Dictionary<AxesPosition, double>();

                //AxesPosition Pmax = CloneHelper.Clone<AxesPosition>(Start_Pos);
                //Pmax.ItemCollection.Find(item => item.Name == "TPSX").Position = xList[maxIndex];
                //Pmax.ItemCollection.Find(item => item.Name == "TPSY").Position = yList[maxIndex];
                //Pmax.ItemCollection.Find(item => item.Name == "TPSZ").Position = zList[maxIndex];
                ...
                ////PD电流
                //maxPoint.Add(Pmax, CalcPD_Current_mA(pList[maxIndex]));

                //AnalyzeResult = maxPoint;
                //return true;
            }
            catch (Exception ex)
            {
        ...
        public Dictionary<AxesPosition, double> Run_Cross(eRunSize_Table eSize, AxesPosition Position, string path)
        {
            ...
            if (false)//TestRecipe.Cross_Times != 0)
            {
                ...
                //    var zstart = new AxesPosition();
                //    var zend = new AxesPosition();
                ...
                //    zstart = CloneHelper.Clone<AxesPosition>(Center);
                //    zend = CloneHelper.Clone<AxesPosition>(Center);
                //    zstart.ItemCollection.Find(item => item.Name == "TPSZ").Position -= TestRecipe.Cross_Width;
                //    zend.ItemCollection.Find(item => item.Name == "TPSZ").Position += TestRecipe.Cross_Width;

                //    rtn = Parallel_MoveLine(axisDict, zstart, zend, Trajspeed, out result, SpeedLevel.Normal);
                //    //rtn = Parallel_MoveLine(axisDict, zstart, zend, TestRecipe.Fine_Trajspeed, out result, SpeedLevel.Normal);
                ...
                //    {
                //        strb = PrintCSV(result);
                //        var sw0 = new StreamWriter(path + $@"\十字线扫Z{i}_{DateTime.Now:yyyyMMdd_HHmmss}.csv");
                //        sw0.Write(strb.ToString());
                //        sw0.Close(); strb.Clear();
                //    }
                //}
