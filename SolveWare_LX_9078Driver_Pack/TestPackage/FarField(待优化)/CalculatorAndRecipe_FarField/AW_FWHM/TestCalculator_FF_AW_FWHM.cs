...
{
    public class TestCalculator_FF_AW_FWHM : TestCalculatorBase
    {
        public TestCalculator_FF_AW_FWHM() : base()
        {
        }

        public CalcRecipe_FF_AW_FWHM CalcRecipe { get; private set; }

        public override Type GetCalcRecipeType()
        {
            return typeof(CalcRecipe_FF_AW_FWHM);
        }

        public override void Localization(ICalcRecipe testRecipe)
        {
            CalcRecipe = (CalcRecipe_FF_AW_FWHM)(testRecipe);
        }
        ...
        public override void Run(IRawDataBaseLite rawData, ref List<SummaryDatumItemBase> summaryDataWithoutSpec, CancellationToken token)
        {
            try
            {
                ...
                dataSummary.Name = string.Format(rawData.RawDataFixFormat, $"{CalcRecipe.CalcData_PreFix}AW_FWHM{CalcRecipe.CalcData_PostFix}");
                ...
                var ret = this.CalculateAngleWidth(angle, newP, 0.5* maxPower);
                dataSummary.Value = ret;
                summaryDataWithoutSpec.Add(dataSummary);
            }
            catch (Exception ex)
            {
                this._core.ReportException($"{this.Name} - {ex.Message}", ErrorCodes.Calc_FF_AW_FWHM_ParamError, ex);
            }
        }
