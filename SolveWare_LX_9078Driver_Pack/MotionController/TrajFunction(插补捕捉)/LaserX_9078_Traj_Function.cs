...
namespace SolveWare_TestPackage
{
    public partial class LaserX_9078_Traj_Function
    {
            ...
            )
        {
            ...
            //rc = LaserX_9078_Utilities.P9078_TrajDelay(CardNo, 50 / 1000);       //不捕捉
            ...
            //等待插补完成 并且获取数据
            rtn = Traj_Run(CardNo, axisDict, token, out result);
            if (rtn != ErrorCodes.NoError)
            {
                //出现异常
                TrajAxisGroup_Close(axisDict.First().Value);
            ...
            )
        {
            try
            {
                ...
                switch (RunPlane)
                {
                    case LaserX_9078_Utilities.PmTrajSelectPlane.XY_CW:
                        {
                            target_P1.x = center.x - Interval / 2;
                    ...
                    case LaserX_9078_Utilities.PmTrajSelectPlane.YZ_CW:
                        {
                            target_P1.y = center.y - Interval / 2;
                            target_P1.z = center.z;
                ...
                if (InSideToOutSide == true)
                {
                    ...
                    rc = LaserX_9078_Utilities.P9078_TrajDelay(CardNo, 50 / 1000);       //等待
                    if (rc != 0) err++;
                        ...
                        do
                        {
                            ...
                            for (int n = 0; n < actCount; n++)
                            {
                                ...
                                foreach (var item in axis9078)
                                {
                                    ...
                                    Traj_CaptionResult.MotorPos_mm[item.Value].Add(Math.Round(_pos[item.Value.AxisNo], 6));
                                }
